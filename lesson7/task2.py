...
class Car:
    ...
    def __str__(self):
        return (f"цвет={self.color}, "
                f"количество поссажирских мест={self.count_passenger_seats}, "
                f"детское кресло={self.is_baby_seat}"
                f"")
...
class Taxi:
    ...
    def find_car(self, count_passengers: int, is_baby: bool) -> Car:
        for car in self.cars:
            if car.is_busy is False:
                if car.count_passenger_seats >= count_p:
